...
import { createBubbleElement, setBubbleTheme } from "./elements/bubble.js";
import { createLiveRegion } from "./elements/live-region.js";
import { createPanel } from "./elements/panel.js";
import { resolveSlot } from "./elements/slot.js";
import { resolveOptions, sameOptions } from "./options.js";
import { assertPanelLength } from "./panel-length.js";
import { resolveTheme, systemThemeName } from "./theme.js";
export { bubbleThemes } from "./theme.js";
/**
 * Mounts the bubble overlay into the document and returns a manager
 * for adding and removing bubbles.
 */
export const createBubbles = (options) => {
    ...
    const occurrences = [];
    let lastState = config.initialState;
    let lastActive;
    let flushQueued = false;
    const currentState = () => group?.state() ?? config.initialState;
    const deliver = (event, detail) => {
    ...
    const flush = () => {
        flushQueued = false;
        for (const { event, detail } of occurrences.splice(0))
        ...
        if (state !== lastState) {
            ...
            if (state === "open")
                ...
                live.announce("Bubbles collapsed");
        }
    ...
    const themeTokens = () => resolveTheme(config.theme === "auto" ? systemThemeName() : config.theme, config.colors);
    // Per-bubble sizing overrides win over the manager's values, and
    // being absolute they survive configure() repaints unchanged.
    const panelAppearance = (overrides) => ({
        theme: themeTokens(),
    ...
    const removeById = (id, reason) => {
        ...
        for (const teardown of bubble.teardowns ?? [])
            teardown();
        bubbles.delete(id);
        retiring.delete(id);
    ...
    return {
        add(options) {
            ...
            if (existing) {
                ...
                if (options.label)
                    existing.el.setAttribute("aria-label", options.label);
                return true;
            }
            if (bubbles.size - retiring.size >= config.maxBubbles)
                return false;
            ...
            const teardowns = [icon.teardown, content.teardown].filter((teardown) => teardown !== undefined);
