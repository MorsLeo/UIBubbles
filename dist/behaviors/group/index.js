...
import { EDGE_MARGIN, Z_BUBBLE_TOP } from "../../constants.js";
import { setBubbleHover, setBubblePressed } from "../../elements/bubble.js";
import { TOUCH_CHASE_RATE } from "../../physics/config.js";
...
export const createBubbleGroup = (callbacks, config) => {
    ...
    const glideMembersToSlots = () => {
        syncZOrder();
        for (const m of members) {
            if (retiring.has(m.id) || m.id === rowDraggingId)
                continue;
            cancelMotion(m.id);
            motions.set(m.id, startGlide(m.el, () => slotTargetFor(m), {
    ...
    const seedMotion = (member, velocity) => {
        cancelMotion(member.id);
        motions.set(member.id, startGlide(member.el, () => slotTargetFor(member), {
    ...
    const handOffActivePanel = (leavingId) => {
        if (activeId !== leavingId)
            return;
    ...
    const joinTrail = (member) => {
        const leaderId = flightLeaderId();
        if (!groupInFlight() || !leaderId || leaderId === member.id)
            return false;
    ...
    return {
        ...
        addMember(member) {
            ...
            if (members.length === 1) {
                ...
                motions.set(member.id, startFling(el, { x: 0, y: 0 }, config.ricochet(), () => {
        ...
        retireMember(id, onGone) {
            ...
            motions.set(id, startGlide(member.el, exitTarget, { initialVelocity: exitVelocity, onRest: onGone }));
        },
        restoreMember(id) {
            const member = byId(id);
            ...
            if (gone)
                fadeInIfReduced(member.el);
            if (!joinTrail(member) && !groupInFlight()) {
                settleMembers();
                if (gone) {
                    seedMotion(member, mode === "open"
                        ? { x: 0, y: LAUNCH_SPEED }
        ...
        onOutsidePointer(target) {
            ...
            if (members.some((m) => m.el.contains(node) || m.panel?.contains(node)))
                return;
            collapse();
            docked()[0]?.el.focus();
        },
        ...
        activate(id) {
            ...
            if (groupDragging || rowDraggingId !== undefined)
                return;
            if (mode === "docked") {
        ...
        onDragEnd(id, velocity) {
            ...
            motions.set(leader.id, startFling(leader.el, velocity, config.ricochet(), () => {
