import { clampTop, maxRestTop } from "../behaviors/clamp.js";
import { prefersReducedMotion } from "../behaviors/reduced-motion.js";
import { runSimulation } from "../behaviors/simulate.js";
import { chooseSide, setSnappedSide, sideRestLeft } from "../behaviors/snap.js";
import { EDGE_MARGIN } from "../constants.js";
import { MAX_EDGE_DIP, REST_DISTANCE, REST_VELOCITY } from "../physics/config.js";
import { frictionDecay, projectDistance } from "../physics/friction.js";
 ...
 */
export const startFling = (el, releaseVelocity, restitution, onRest) => {
    ...
    const verticalReturnEdge = rect.top < EDGE_MARGIN ? "top" : rect.top > maxRestTop(el) ? "bottom" : undefined;
    const verticalReturnTarget = () => verticalReturnEdge === "top"
        ? EDGE_MARGIN
    ...
    const maxDipLeft = () => viewportWidth() - el.offsetWidth + MAX_EDGE_DIP;
    let dipCapArmed = rect.left >= minDipLeft && rect.left <= maxDipLeft();
    let x = { position: rect.left, velocity: releaseVelocity.x };
    let y = { position: rect.top, velocity: releaseVelocity.y };
    const stepHorizontal = (dt) => {
        ...
        if (x.position < minDipLeft)
            x = { position: minDipLeft, velocity: 0 };
        else if (x.position > maxDipLeft())
            x = { position: maxDipLeft(), velocity: 0 };
    };
    const stepVertical = (dt) => {
        ...
        else if (y.position >= max)
            y = { position: max, velocity: -y.velocity * restitution };
    };
    const isAtRest = () => {
        const returnTarget = verticalReturnTarget();
        const horizontal = Math.abs(x.position - targetLeft()) < REST_DISTANCE && Math.abs(x.velocity) < REST_VELOCITY;
        const vertical = Math.abs(y.velocity) < REST_VELOCITY &&
