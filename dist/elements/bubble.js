import { Z_BUBBLE_TOP } from "../constants.js";
...
export const setBubbleHover = (el, hovered) => {
    visualControls.get(el)?.setHovered(hovered);
};
export const setBubblePressed = (el, pressed) => {
...
export const setBubbleTheme = (el, theme) => {
...
const createDefaultIcon = (stroke) => {
 ...
 */
const createSurface = (theme, icon) => {
...
export const createBubbleElement = (initialTheme, icon, label) => {
    ...
    themeControls.set(el, (next) => {
        ...
        if (focused)
            surface.style.outlineColor = theme.focusRing;
    });
