import type { PanelLength } from "$src/types";
 ...
 * untyped JavaScript.
 */
export const isPanelLength = (value: unknown): value is PanelLength =>
 ...
 */
export const assertPanelLength = (value: PanelLength | undefined, option: string): void => {
	if (value !== undefined && !isPanelLength(value))
		throw new TypeError(
			`bubbles: ${option} must be a non-negative number (px) or a "<n>px" / "<n>%" string, received ${JSON.stringify(value)}`
		);
};

/** Formats a validated panel dimension as a CSS length; a bare number is px. */
export const toCssLength = (value: PanelLength): string =>
