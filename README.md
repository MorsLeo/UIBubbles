...
Using Svelte? [`@hyperplexed/bubbles-svelte`](https://github.com/githyperplexed/bubbles/tree/main/packages/svelte) wraps all of this in declarative components — `<Bubbles>`/`<Bubble>` with snippets for icon and panel content.
...
The repo is a bun workspace: this library lives at `packages/core`, the Svelte wrapper at `packages/svelte`, and the playground site and e2e tests at the root. All commands run from the repo root:
...
Only each package's `dist/` (plus non-test `src/`) is published.
